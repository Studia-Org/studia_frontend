    ...
    function renderPostsInside(post) {
        return (
            <div className='text-sm w-full'>
                ...
                <p className='text-gray-700 mt-2 line-clamp-3'>{post.attributes.content}</p>
                <div className='flex items-center  mt-3 w-full '>
                    <img src={post.attributes.autor.data.attributes.profile_photo.data.attributes.url} className='w-8 rounded-full' alt="" />
                    <p className='font-medium ml-1'>{post.attributes.autor.data.attributes.name}</p>
                    <span className='flex items-center space-x-1 text-black ml-auto font-medium'><FiCornerDownLeft /> <p className='ml-auto'> {post.attributes.forum_answers.data.length} Replies</p></span>
                </div>
                <hr className='mt-4' />
            </div>
        )
        ...
        <div className='flex-shrink-0 w-full sm:w-auto'>
            <div className='mt-4 bg-white rounded-lg  px-5 py-5  sm:mr-9 sm:right-0 sm:w-[30rem] w-full shadow-md sm:visible collapse'>
                <div className='flex items-center'>
                    ...
                    <button onClick={setForumFlag} className='text-base ml-auto font-medium text-indigo-700'>View all posts</button>
