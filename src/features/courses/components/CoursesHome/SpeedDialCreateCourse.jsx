...
export const SpeedDialCreateCourse = ({ setExpandCreateCourseStudent }) => {
    ...
    return (
        <div className='fixed flex right-[6.5rem] bottom-12'>
            <div
                id="speed-dial-menu-dropdown"
                className={`bg-white shadow rounded-t-2xl rounded-bl-2xl transform scale-0 -translate-x-6 opacity-0 mb-5 h-3 duration-200 ${isExpanded ? 'scale-100 h-[10rem] opacity-100' : ''}`}
            >
            ...
            <button
                ...
                className="flex items-center justify-center w-[3.2rem] h-[3.2rem]  text-white transition bg-[#3c3c3c] rounded-full shadow-xl hover:bg-[#4f4f4f] duration-100 hover-scale active-scale "
                onClick={() => handleClick()}
            >
                <FiPlus size={26} />
