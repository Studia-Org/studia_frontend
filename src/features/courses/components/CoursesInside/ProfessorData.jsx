...
export const ProfessorData = ({ professor, evaluatorFlag }) => {
    ...
    const id = professor.attributes.id || professor.id
    link = '/app/profile/' + id + '/';
            ...
            <div className={`${evaluatorFlag ? 'mt-0 ' : 'my-4'}
             flex flex-col bg-white rounded-lg  px-5 py-5  sm:right-0 md:max-w-[30rem] shadow-md   `}>
                <div className='flex items-center'>
                    {
                        evaluatorFlag === true ?
                            <p className='text-lg font-medium'>{t("ACTIVITY.about_evaluator")}</p> :
                            <>
                                ...
                                <p className='text-lg font-medium'>{t("ACTIVITY.about_professor")}</p>
                            </>

                    }
                    <a target='_blank' href={link} rel='noreferrer noopener'
                        className='ml-auto text-base font-medium text-indigo-700'>{t("ACTIVITY.view_profile")}</a>
                ...
                <div className='flex items-center my-4 space-x-3'>
                    {
                        professor.attributes.profile_photo.url ?
                            <img className='w-[3rem] h-[3rem] rounded' src={professor.attributes?.profile_photo?.url} alt="" />
                            :
                            <img className='w-[3rem] h-[3rem] rounded' src={professor.attributes?.profile_photo?.data?.attributes?.url} alt="" />
                    }
